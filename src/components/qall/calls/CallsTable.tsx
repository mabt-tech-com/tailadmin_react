...
const CallsTable: React.FC = () => {
  ...
  const { getCalls, searchCalls, getCallRecordings, getCallTranscripts, error } = useCallsApi();
  ...
  useEffect(() => {
      ...
      try {
        ...
        const transformedCalls = await Promise.all(callsData.map(async (call) => {
          ...
          try {
            ...
            console.error(`Error fetching additional data for call ${call.id}:`, err);
          }
          ...
          return {
            ...
            performancePercent: call.metadata?.confidence ? Math.round(call.metadata.confidence * 100) : null,
    ...
    fetchCalls();
  }, [page, selectedTab, searchQuery, getCalls, searchCalls, getCallRecordings, getCallTranscripts]);
